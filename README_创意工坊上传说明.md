...
- v1.16.71：仓库核心宿主恢复为正常网络容器，安装不再要求清空或锁定容量；旧 v1.16.70 宿主会一次性恢复原容量。仓库页重做为人物库存、网络仓库和详情三栏，支持当前人物容器来源、折叠分组、Ctrl/Shift 多选、一个/一半/全部存取及装备失败恢复。连接标识改为绿色核心宿主、蓝色已连通容器、红色未连通标记，同格对象稳定编号并支持右键临时高亮。该版本仍需完成 B42.19 SP/MP 实机后才能作为稳定版发布。
