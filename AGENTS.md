...
- Develop and verify in Git first. After automated checks pass, copy only the packaged runtime files to the Workshop test directory and verify the deployed files. Wait for the user's game-test result before pushing the completed branch or treating it as a release baseline.
