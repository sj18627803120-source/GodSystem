...
Require-Text $carry 'getMaxWeightDelta' 'Carry bonus must read the native delta'
Require-Text $carry 'setMaxWeightDelta' 'Carry bonus must write the native delta'
Require-Text $carry 'getVanillaCapacity' 'Carry bonus must measure the B42 vanilla capacity'
Require-Text $carry 'desiredFinal\s*=\s*nativeFinal\s*\+\s*desiredBonus' 'Carry units must be added to the vanilla final'
...
Require-Text $carry 'GodSystemCarryNativeDelta' 'Carry refresh must preserve the native multiplier marker'
...
Require-Text $ui 'function\s+GodSystemWindow:populateUpgrades\(\)[\s\S]*local upgrades = \{ "carryCapacity" \}' 'Carry capacity must remain in the general upgrades page'
Require-Text $ui 'function\s+GodSystemWindow:populateTaskExtensions\(\)[\s\S]*local upgrades = \{ "activeTasks", "dailyTasks" \}' 'Task limits must appear in the task extensions page'
