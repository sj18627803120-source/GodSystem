...
local function moveableObjectAt(x, y, z, spriteName)
    local square = squareAt(x, y, z)
    local object = { data = {}, square = square, spriteName = spriteName }
    ...
    function object:getContainerCount() return 0 end
    square.objects.values[#square.objects.values + 1] = object
    return object
end

local function containerObjectAt(x, y, z, slotValues)
    local object = moveableObjectAt(x, y, z, "test_storage_sprite")
    object.slots = {}
...
assert(Storage.isController(controllerItem), "the custom inventory item must be recognized as a controller")

local controllerObject = moveableObjectAt(0, 0, 0, "GodSystem_StorageController_0")
controllerObject.data.movableData = {
    [Storage.ControllerNetworkKey] = controllerItem.data.movableData[Storage.ControllerNetworkKey],
    [Storage.ControllerTokenKey] = controllerItem.data.movableData[Storage.ControllerTokenKey],
    [Storage.ControllerInstalledKey] = true,
}
assert(Storage.isController(controllerObject),
    "the placed custom Moveable must be recognized from copied movableData")

local foundObject, foundItem = Storage.findWorldController(0, 0, 0, controllerItem:getID(), "token-a")
assert(foundObject == controllerObject and foundItem == nil,
    "the placed custom Moveable must be discoverable without a world inventory item")
...
assert(view.nodeCount == 1, "the adjacent marked crate must connect to the custom controller")
...
assert(snapshot.itemCount == 1 and snapshot.groupCount == 1, "pre-existing contents must be indexed")
...
print("Test-GodSystemV11669Runtime passed")
