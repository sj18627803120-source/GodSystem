...
Require-Text $workshop ('v' + $version) 'Workshop usage summary must cover the current release'
Reject-Text $workshop '(?m)^description=v1\.16\.68' 'Workshop description must not contain the old version dump'
Require-Text $readme ('GodSystemWorkshopUpload_v' + $version) 'Full player usage guide is missing'
...
if (-not $luaExe) { throw "Lua 5.1 runtime is required for v$ExpectedVersion validation" }
...
if ($LASTEXITCODE -ne 0) { throw "v$ExpectedVersion storage routing runtime test failed" }
