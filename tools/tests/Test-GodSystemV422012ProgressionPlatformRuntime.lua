...
local function stateChecks()
    ...
    medical.public.save(actor, { stats = { spentPoints = 7 }, marker = "kept" })
    equal(medical.public.load(actor).marker, "kept", "medical state persistence")
    equal(medical.public.load(actor).stats, nil, "medical state retained shared metrics")
