...
Require-Text $capacity '__index\.getEffectiveCapacity' 'Effective-capacity wrapper is missing'
Require-Text $capacity '__index\.hasRoomFor' 'Room-check wrapper is missing'
Require-Text $capacity 'GodSystem\.SystemSpaceTerminal' 'Override must identify only the GodSystem terminal'
Require-Text $capacity 'return\s+originalEffectiveCapacity\s*\(' 'Non-terminal effective capacity must call the original method'
Require-Text $capacity 'return\s+originalHasRoomFor\s*\(' 'Non-terminal room checks must call the original method'
Require-Text $capacity 'setmetatable\([^\r\n]+__mode\s*=\s*"k"' 'Runtime container map must use weak keys'
Require-Text $capacity 'MAX_CAPACITY\s*=\s*2000' 'Override must enforce its own hard boundary'
