...
def parse_admin_meta() -> list[dict[str, str]]:
    ...
    for raw in ADMIN_CONFIG_PATH.read_text(encoding="utf-8").splitlines():
        ...
        rows.append(fields)
    if len(rows) != 75:
        raise ValueError(f"Expected 75 admin settings, found {len(rows)}")
    return rows
...
def main() -> None:
    ...
    write_sandbox_files()
    print(f"updated {len(ui_entries)} UI keys, {len(item_entries)} item keys, and {len(parse_admin_meta())} sandbox options")
