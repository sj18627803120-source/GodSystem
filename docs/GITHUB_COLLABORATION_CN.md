...
1. 记录用户实际测试的游戏版本、覆盖范围、明确反馈和仍未逐项验证的场景；“未发现明显问题”不能改写为所有 SP/MP 压力项都通过。
2. 从已测试的 Git worktree 重新运行 `tools\Test-GodSystem.ps1` 和 `git diff --check`，确认本地化生成文件、版本号和运行时文件仍一致。
3. 只提交当前功能分支，推送后用 `git ls-remote --heads origin <branch>` 对照本地 `HEAD`；不要推送临时日志、Workshop 缓存、备份 ZIP 或游戏存档。
4. 除非用户单独授权，发布 Workshop 不自动触发合并 `main`、创建标签、覆盖稳定备份或改写历史。功能分支是本次发布代码的可追溯来源。
