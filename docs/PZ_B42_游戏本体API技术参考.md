...
## 二十二、GodSystem UI 状态恢复实践（工程注记）

> 本节记录 GodSystem 在 B42.20 上的 UI 工程经验，不是新增或已验证的原版 Java/Kahlua API 结论。具体方法签名仍以当前 B42 本体 Lua 调用点和实机为准。

- `ISScrollingListBox` 的滚动位置、选中项、详情和动作按钮是分离状态。列表重建后按稳定业务 key 恢复位置与选择；不要让同步占位或空列表覆盖等待中的恢复快照。
- `clear()` 或重新填充列表不应被当作“自然回到顶部”的许可。相同逻辑上下文的购买、回收、存取、隐藏、整理和状态回包都应恢复原滚动区域；页面、分区、搜索、分类、排序或筛选改变时才显式重置。
- 动作按钮文本必须从当前页面分区与当前 payload 正向计算。例如任务升级行应明确写入“购买升级”，不能只依赖初始化阶段曾写过的按钮标题；延迟刷新可能保留普通任务行的旧文本。
- 右键菜单的功能仍应在关键场景提供可见入口。它可以加速操作，但不应成为购买套餐或其他核心玩家能力的唯一发现方式。
