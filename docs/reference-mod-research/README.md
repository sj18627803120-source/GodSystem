...
研究库只提供机制摘要、相对文件路径、接口名称和工程判断，不包含第三方 MOD 源码或资源。需要实现功能时，先读 [B42 官方开发资料与适用边界](../PZ_B42_OFFICIAL_DEVELOPMENT_CN.md) 和 [社区开发资源核验](../PZ_COMMUNITY_DEVELOPMENT_RESOURCES_CN.md)，再核对目标游戏版本的原版文件，最后把这里的社区实现作为补充证据。

## 使用顺序

1. 先在[官方开发资料](../PZ_B42_OFFICIAL_DEVELOPMENT_CN.md)确认当前规则和版本边界。
2. 在[来源目录](catalog.md)确认参考 MOD 的目标版本和证据强度。
3. 按下面的主题索引选择来源报告。
4. 在报告的“关键证据”中确认真实文件和符号。
5. 区分代码事实、作者声明、工程推断和待实机项目。
6. 跨项目稳定结论以 `tools/codex/skills/pz-mod-dev/references/pz-b42-patterns.md` 为准。
